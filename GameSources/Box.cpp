...
namespace basecross
{
	...
	void Box::BoxMove()
	{
		...
		if (moveDirection.length() > 0.0f)
		{
			...
			if (stageptr)
			{
				table = stageptr->GetTableObject(); // GameStageからTableオブジェクトを取得
			}
			else if (stageptr2)
			{
				table = stageptr2->GetTableObject(); // GameStageからTableオブジェクトを取得
			}
			else if (stageptr3)
			{
				table = stageptr3->GetTableObject(); // GameStageからTableオブジェクトを取得
			}


			if (table)
			{
				ClampObjectPositionToTable(nextPos, boxScaleHalved, table);
	...
	void Box::ClampObjectPositionToTable(Vec3 nextPos, Vec3 boxScaleHalved, std::shared_ptr<GameObject>& table)
	{
		if (!table)
		{
			return;
